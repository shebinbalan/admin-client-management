...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Edit Client: {{ $client->name }}</h1>
        ...
        <form method="POST" action="{{ route('admin.clients.update', $client) }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                @if($client->profile_image)
                    <div class="mb-2">
                        <img src="{{ $client->profile_image_url }}" alt="Current Profile" class="img-thumbnail" style="width: 100px;">
                    </div>
                @endif
                <input type="file" class="form-control @error('profile_image') is-invalid @enderror" 
                       id="profile_image" name="profile_image" accept="image/*">
                <div class="form-text">Max size: 2MB, Max resolution: 1920x1080px</div>
                @error('profile_image')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
